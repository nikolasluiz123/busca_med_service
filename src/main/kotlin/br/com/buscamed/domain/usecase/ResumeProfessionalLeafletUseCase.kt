...
/**
 * Caso de uso responsável por recuperar, processar e resumir a bula destinada a profissionais de saúde de um medicamento específico.
 *
 ...
 * @property pdfProcessService Serviço de processamento na LLM especializado em bulas de profissionais.
 * @property executionHistoryRepository Repositório de histórico de execução da LLM.
 */
class ResumeProfessionalLeafletUseCase(
    ...
    /**
     * Executa o processamento da bula de profissionais para o medicamento informado.
     *
    ...
    suspend operator fun invoke(medicationId: String, autoSave: Boolean = false): AnvisaMedicationLeaflet? = withContext(Dispatchers.IO) {
        val leaflet = repository.findLeafletBy(medicationId, AnvisaMedicationLeafletIdentifier.PROFESSIONAL)

        if (leaflet == null || leaflet.leafletStoragePath == null || leaflet.leafletResume != null) {
            return@withContext null
        }
        
        logger.info("Iniciando extração e processamento da bula de profissionais para o medicamento GGREM [{}].", medicationId)
        ...
        try {
            ...
            if (autoSave) {
                repository.saveLeaflets(
                    ...
                    leafletIdentifier = AnvisaMedicationLeafletIdentifier.PROFESSIONAL
                )
            }
            
            logger.info("Processamento da bula de profissionais [{}] concluído com sucesso.", medicationId)
            updatedLeaflet
        } catch (e: Exception) {
            executionSuccess = false
            logger.error("Erro durante o processamento da bula de profissionais [{}]. Falha repassada ao histórico.", medicationId)
            throw e
        } finally {
